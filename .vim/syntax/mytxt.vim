...
syn match   ttSeparator		"^━━\+$"
syn match   ttSeparator		"^==\+$"
syn match   ttTime			"^[\s\t　]*\d\{2}:\d\{2}:\d\{2}"
syn match   ttTime			"\d\{2}:\d\{2}:\d\{2}" contained
syn match   ttDate			"^[\s\t　]*\d\{4}/\d\{2}/\d\{2}"
syn match   ttDate			"^[\s\t　]*\d\{4}/\d\{2}/\d\{2}[\s\t　]\+\d\{2}:\d\{2}:\d\{2}" contains=ttTime
syn match   ttKey			"[\s\t　][0-9A-Z]\{8}$" contained
syn match   ttLink			"→.*[0-9A-Z]\{8}$"
syn match   ttHeader1		"^□.\+" contains=ttKey
syn match   ttHeader2		"[^→]\+[\s\t　][0-9A-Z]\{8}$" contains=ttKey
